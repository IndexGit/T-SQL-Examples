...
--  Procedure checks type of data in incoming @xml and returns all error if exists in output xml @errors
-------------------------------------------------------------------------------------
--	Автор:	Мамонов А.В.
--	Модуль:	Общее
--	Дата:	10/07/2013
...
/*
		Типы ошибок:
			-1 - не найден тип в системе, no data type founded in system
			 0 - нет ошибки, no errors
			 2 - Требуется значение, value expected
			 3 - Ошибочный тип данных , data type error
			 4 - Проверка переполнения в типе, error overflow 
*/
